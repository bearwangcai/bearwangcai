...
def file_name(file_dir):   
    ...
    num = len(files_name)              
    return  files_name, num #都有哪些省，文件路径，文件总数
...
def read(file_name):
    ...
    for index,paragraph in enumerate(document.paragraphs):
        if paragraph.text in province_name:
            ...
            try:
                province[province_name_now][content_name_now].add(paragraph.text) #填写内容
            except:
                pass
    return province
...
if __name__ == "__main__":
    ...
    files_name, num = file_name(file_dir)

    print(num) #文件总数

    for i in range(num): #读文件
        profession['IDC'] = read(files_name[i])
